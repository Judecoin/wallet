...
Now that the daemon is synced, open the extracted folder, right-click in an empty area, select `Open in Terminal`, and run judecoin-wallet-cli:

```
.\judecoin-wallet-cli.exe
```

If the language selection display freezes, exit with `Ctrl-C`, then run again with:
...
- When typing the password, the characters will not appear. It will seem as if you are typing and no text is appearing however the terminal is logging every character you are typing including if it is capitalised or lowercase.
